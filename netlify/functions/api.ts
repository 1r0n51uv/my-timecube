...
function getRoute(pathname: string) {
  const match = pathname.match(/\/users\/([^/]+)\/(timesheets|activities)$/);
...
async function handleActivities(username: string, req: Request) {
  ...
  if (req.method === "GET") {
    ...
    const codes = activities.length > 0 ? activities.map((activity) => activity.code) : DEFAULT_ACTIVITIES;
    return json({ activities: codes });
  }
